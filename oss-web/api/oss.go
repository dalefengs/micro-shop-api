...
func Callback(ctx *gin.Context) {
	...
	if err != nil {
		response.FailResponse(ctx, http.StatusBadRequest, status2.InvalidParameter, err.Error(), "")
		return
	}
...
func get_policy_token() PolicyToken {
	now := time.Now().Unix()
	expire_end := now + global.Config.Aliyun.ExpireTime
	...
	condition = append(condition, global.Config.Aliyun.UploadDir)
	config.Conditions = append(config.Conditions, condition)

	//calucate signature
	result, err := json.Marshal(config)
	debyte := base64.StdEncoding.EncodeToString(result)
	h := hmac.New(func() hash.Hash { return sha1.New() }, []byte(global.Config.Aliyun.AccessKeySecret))
	io.WriteString(h, debyte)
	signedStr := base64.StdEncoding.EncodeToString(h.Sum(nil))

	var callbackParam CallbackParam
	callbackParam.CallbackUrl = global.Config.Aliyun.CallbackUrl
	...
	policyToken.AccessKeyId = global.Config.Aliyun.AccessKeyId
	policyToken.Host = global.Config.Aliyun.Bucket
	policyToken.Expire = expire_end
	policyToken.Signature = string(signedStr)
	policyToken.Directory = global.Config.Aliyun.UploadDir
